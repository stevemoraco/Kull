 ...
 * Tests for Kimi K2 model, vision support, 1-1000 ratings, RAW reminders, and correct model name
 */
...
describe('GroqAdapter', () => {
  ...
  describe('Model Configuration', () => {
    it('should use correct model name moonshotai/kimi-k2-instruct-0905', () => {
      expect((adapter as any).modelName).toBe('moonshotai/kimi-k2-instruct-0905');
    });

    it('should use correct pricing $0.20/$0.50 per 1M tokens', () => {
      const INPUT_COST = (adapter as any).INPUT_COST_PER_1M;
      const OUTPUT_COST = (adapter as any).OUTPUT_COST_PER_1M;

      expect(INPUT_COST).toBe(0.20);
      expect(OUTPUT_COST).toBe(0.50);
    });

    it('should return Groq as provider name', () => {
  ...
  describe('Vision Support', () => {
    ...
    it('should include high detail parameter in image URL', async () => {
      const request: ProcessImageRequest = {
        image: mockImage,
        systemPrompt: 'Rate',
        userPrompt: 'Analyze'
      ...
      // Verify image_url structure exists (high detail is implicit in base64 format)
      expect(body.messages[1].content[0].image_url).toBeDefined();
      expect(body.messages[1].content[0].image_url.url).toContain('base64');
    });
  });

  describe('1-1000 Scale Rating Fields', () => {
    it('should return all 9 required technicalQuality fields (1-1000 scale)', async () => {
      const request: ProcessImageRequest = {
        image: mockImage,
        systemPrompt: 'Rate',
        userPrompt: 'Analyze'
      ...
      // Verify all 9 technicalQuality metrics
    ...
    it('should return all 7 required subjectAnalysis numeric fields (1-1000 scale)', async () => {
      const request: ProcessImageRequest = {
        image: mockImage,
        systemPrompt: 'Rate',
        userPrompt: 'Analyze'
      ...
      // Verify all 7 numeric subjectAnalysis metrics
    ...
    it('should handle optional shootContext fields', async () => {
      const request: ProcessImageRequest = {
        image: mockImage,
        systemPrompt: 'Rate',
        userPrompt: 'Analyze'
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: async () => ({
          ...
          usage: { prompt_tokens: 2300, completion_tokens: 380 }
        })
      } as any);

      const result = await adapter.processSingleImage(request);

      expect(result.rating.shootContext).toBeDefined();
      expect(result.rating.shootContext?.eventType).toBe('wedding');
      expect(result.rating.shootContext?.shootPhase).toBe('ceremony');
      expect(result.rating.shootContext?.timeOfDay).toBe('afternoon');
      expect(result.rating.shootContext?.location).toBe('church');
    });

    it('should validate ranges for 1-1000 fields (clamp if out of range)', async () => {
      const request: ProcessImageRequest = {
        image: mockImage,
        systemPrompt: 'Rate',
        userPrompt: 'Analyze'
      ...
      // Return values outside valid range to test clamping
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          choices: [{
            message: {
              content: JSON.stringify({
                ...
                tags: [],
                technicalQuality: {
                  focusAccuracy: 1500,  // Too high, should clamp to 1000
                  exposureQuality: -100, // Too low, should clamp to 1
                  compositionScore: 500,
                  lightingQuality: 500,
                  colorHarmony: 500,
                  noiseLevel: 500,
                ...
                subjectAnalysis: {
                  primarySubject: 'Test',
                  emotionIntensity: 2000, // Too high, should clamp to 1000
                  eyesOpen: true,
                  eyeContact: false,
                  genuineExpression: 0,    // Too low, should clamp to 1
                  facialSharpness: 500,
                  bodyLanguage: 500,
                  momentTiming: 500,
                  storyTelling: 500,
      ...
      // Verify clamping
      expect(result.rating.technicalQuality.focusAccuracy).toBe(1000); // Clamped from 1500
      expect(result.rating.technicalQuality.exposureQuality).toBe(1);  // Clamped from -100
      expect(result.rating.subjectAnalysis.emotionIntensity).toBe(1000); // Clamped from 2000
      expect(result.rating.subjectAnalysis.genuineExpression).toBe(1); // Clamped from 0
    });

    it('should use default values (500) for missing 1-1000 fields', async () => {
      const request: ProcessImageRequest = {
        image: mockImage,
        systemPrompt: 'Rate',
        userPrompt: 'Analyze'
      ...
      expect(result.rating.technicalQuality.exposureQuality).toBe(500);
      expect(result.rating.technicalQuality.compositionScore).toBe(500);
      expect(result.rating.technicalQuality.lightingQuality).toBe(500);
      expect(result.rating.technicalQuality.colorHarmony).toBe(500);
      expect(result.rating.technicalQuality.noiseLevel).toBe(500);
      expect(result.rating.technicalQuality.sharpnessDetail).toBe(500);
      expect(result.rating.technicalQuality.dynamicRange).toBe(500);
      ...
      expect(result.rating.subjectAnalysis.genuineExpression).toBe(500);
      expect(result.rating.subjectAnalysis.facialSharpness).toBe(500);
      expect(result.rating.subjectAnalysis.bodyLanguage).toBe(500);
      expect(result.rating.subjectAnalysis.momentTiming).toBe(500);
      expect(result.rating.subjectAnalysis.storyTelling).toBe(500);
      expect(result.rating.subjectAnalysis.uniqueness).toBe(500);
  ...
  describe('RAW Image Reminder', () => {
    ...
    it('should note that exposure quality is fixable in RAW', async () => {
      const request: ProcessImageRequest = {
        image: mockImage,
          ...
          choices: [{
            message: {
              content: JSON.stringify({
                ...
                technicalQuality: {
                  focusAccuracy: 850,
                  exposureQuality: 400, // Low but fixable in RAW
                  compositionScore: 800,
                  lightingQuality: 700,
                  colorHarmony: 650,
                  noiseLevel: 750,
                  sharpnessDetail: 830,
                  dynamicRange: 900,
                  overallTechnical: 750
                },
                subjectAnalysis: {
                  primarySubject: 'Person',
      ...
      // Verify RAW reminder mentions exposure is correctable
      expect(systemContent).toContain('Exposure and white balance are FULLY correctable');
    });
  });

  describe('Cost Calculation', () => {
    it('should calculate cost correctly with $0.20/$0.50 per 1M tokens', async () => {
      const request: ProcessImageRequest = {
        image: mockImage,
        systemPrompt: 'Rate',
        userPrompt: 'Analyze'
      ...
      // 1M input * $0.20 = $0.20
      expect(result.cost.inputCostUSD).toBe(0.20);
      // 0.5M output * $0.50 = $0.25
      expect(result.cost.outputCostUSD).toBe(0.25);
      // Total = $0.45
      expect(result.cost.totalCostUSD).toBe(0.45);
      // User charge = 2x = $0.90
      expect(result.cost.userChargeUSD).toBe(0.90);
    });
    ...
    it('should estimate cost per image correctly', () => {
      ...
      // (2000 / 1M) * $0.20 + (300 / 1M) * $0.50
      // = 0.0004 + 0.00015 = 0.00055
      expect(estimatedCost).toBeCloseTo(0.00055, 5);
    });
    ...
    it('should track token usage accurately', async () => {
      ...
      expect(result.cost.inputTokens).toBe(3456);
      expect(result.cost.outputTokens).toBe(789);
    });
  });
  ...
  describe('Error Handling', () => {
    ...
    it('should throw error for missing content in response', async () => {
      ...
      await expect(adapter.processSingleImage(request)).rejects.toThrow('No content in Groq response');
    }, 10000);
    ...
    it('should handle API errors with status codes', async () => {
      ...
      await expect(adapter.processSingleImage(request)).rejects.toThrow('Groq API error');
    }, 10000);
  });
  ...
  describe('Batch API', () => {
    it('should throw error for batch processing (not supported)', async () => {
      ...
      await expect(adapter.submitBatch(request)).rejects.toThrow(
        'Groq does not support batch API'
      );
    });
  ...
  describe('Retry and Backoff', () => {
    ...
    it('should respect max retry time (6 hours)', async () => {
      const request: ProcessImageRequest = {
        image: mockImage,
        systemPrompt: 'Rate',
        userPrompt: 'Analyze'
      ...
      await expect(adapter.processSingleImage(request)).rejects.toThrow('Persistent failure');
    }, 10000);
  ...
  describe('Response Processing', () => {
    it('should parse JSON responses correctly', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: async () => ({
          choices: [{
            message: {
              content: JSON.stringify(expectedRating)
            }
          }],
          usage: { prompt_tokens: 2000, completion_tokens: 350 }
        })
      } as any);
      ...
      expect(result.rating.starRating).toBe(4);
      expect(result.rating.description).toBe('Beautiful composition');
      expect(result.rating.tags).toEqual(['landscape', 'golden-hour']);
    });
    ...
    it('should handle missing fields gracefully with defaults', async () => {
      ...
      expect(result.rating.colorLabel).toBe('none');
      expect(result.rating.keepReject).toBe('reject'); // star 2 = reject
      expect(result.rating.tags).toEqual([]);
      expect(result.rating.technicalQuality.focusAccuracy).toBe(500);
      expect(result.rating.subjectAnalysis.primarySubject).toBe('Unknown');
    });
