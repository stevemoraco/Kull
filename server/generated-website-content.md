# Kull Website Content

This content is auto-generated from the actual website codebase.

## Home Page (/)

Kull
Sign In
text-logo
button-login-nav

## Dashboard (/dashboard)

Kull
window.location.href =
Sign In
Start Your Free Trial to Access Downloads
Choose a plan below to start your 24-hour free trial and download Kull for Mac, iPhone, and iPad.
No charge until your trial ends • Cancel anytime
Professional
Perfect for individual photographers
$99
/month
Billed annually at $1,188/year
Save $396 vs monthly billing
handleSelectPlan(
Download Kull Now
Most Popular
Studio
For studios and high-volume photographers
$499
Billed annually at $5,988/year
Save $2,004 vs monthly billing
After selecting a plan, you
All plans can be canceled anytime. No long-term commitments.
button-login-nav
heading-dashboard
card-pricing-professional
button-select-professional
card-pricing-studio
button-select-studio
After selecting a plan, you'll be able to download Kull for Mac, iPhone, and iPad immediately during your free trial.

## Checkout (/checkout)

1 ?
0 && (
You
through referrals. This discount has been applied to your annual price below!
Plus priority support access!
0 ?
0 ? (
) : (
0 
                ?
Cancel
Card Pre-Authorization
By starting your trial, you agree to our Terms of Service and Privacy Policy.
          You can cancel anytime during your 24-hour trial with zero charge.
(null);
  const [bonus, setBonus] = useState
Kull
setLocation(
Back
Start Your Free Trial
24 hours of unlimited AI photo rating. No charge until trial ends.
button-downgrade-monthly
button-cancel-downgrade
button-start-trial
loading-spinner
button-back
text-checkout-headline
You've earned
Loading

## User Home (/home after login)

new Date();

  // Check trial status
  const isOnTrial = typedUser?.subscriptionStatus ===
Kull
Sign Out
Download for Mac
Get the universal Mac app and start rating, organizing, and tagging photos from any folder.
Download DMG File
macOS 11.0 or later required
iPhone & iPad Apps
Rate and organize photos on the go with automatic sync across all devices.
Get on App Store
iOS 15.0 or later required
Refer Friends & Earn Rewards
Invite up to 10 photographers and unlock bonuses from extra features to free months
text-user-email
button-logout
text-welcome-headline
button-download-dmg
button-download-ios

## Refunds (/refund)

(null);
  
  // Survey form state
  const [primaryReason, setPrimaryReason] = useState(
(null);
  const [missingFeature, setMissingFeature] = useState(
(null);
  const audioChunksRef = useRef
Back to Home
Kull
Refund Policy
Last Updated: November 3, 2025
Help Us Improve - Quick Survey
Before we process your refund, we
1. What
Too expensive for my needs
Missing features I needed
Technical problems or bugs
Product wasn
Other reason
2. Would you recommend Kull to other photographers?
setWouldRecommend(val ===
Yes, despite this issue
No, not in its current state
3. What feature or capability were you hoping to find in Kull?
4. Did you experience any technical issues or bugs?
5. Any other feedback you
Voice Feedback
Stop Recording
Transcribing...
setStep(
Back
Refund Processed Successfully!
You will see the credit in your account within 5-7 business days.
Thank you for your valuable feedback. We
Request Instant Refund
Within 7 days of your first payment, you can request an instant refund. We just need a few minutes of your time to understand what didn
Start Refund Request
Our Commitment to Satisfaction
We want you to love Kull. That
Free Trial Period
24-Hour Trial - Zero Risk
All new users receive a full 24-hour trial period. You can cancel anytime during this period with absolutely no charge. This is your risk-free opportunity to test Kull with your own photos.
7-Day Money-Back Guarantee
If you
Request a refund within 7 days of your first payment
Applies to both monthly and annual subscriptions
Full refund after completing a quick feedback survey
Processed within 5-7 business days
How to Request a Refund
Sign In
Make sure you
Complete Quick Survey
Answer 3-5 questions about your experience (takes 2 minutes). You can type or use voice feedback.
Receive Instant Refund
Your refund will be processed instantly and you
Need Help?
Have questions about our refund policy? Use our chat support on any page for instant help.
Lander Media
31 N Tejon St
Colorado Springs, CO 80903
Founded 2014
button-back-home
text-refunds-headline
radio-too-expensive
radio-missing-features
radio-technical-issues
radio-not-as-described
radio-other
radio-recommend-yes
radio-recommend-no
textarea-missing-feature
textarea-technical-issues
button-start-recording
button-stop-recording
textarea-additional-feedback
button-back-to-info
button-submit-survey
button-start-refund
1. What's the main reason you're requesting a refund? *
Product wasn't as described
5. Any other feedback you'd like to share?
Thank you for your valuable feedback. We'll use it to improve Kull!
Within 7 days of your first payment, you can request an instant refund. We just need a few minutes of your time to understand what didn't work for you.
We want you to love Kull. That's why we offer a generous 1-day free trial and a fair refund policy for new subscribers.
If you're not satisfied with Kull after your trial ends, we offer a 7-day money-back guarantee on your first payment:
Make sure you're logged into your account.
Your refund will be processed instantly and you'll see the credit in 5-7 business days.
e.g., Support for video rating, batch export, specific AI model, etc.
e.g., Installation problems, crashes, slow performance, sync issues, etc.
Anything else we should know? You can type here or use the voice feedback button above.

## Key Components

### Hero
1-Day Unlimited Free Trial
Rate 1,000+ Photos in Minutes
Using AI
Universal Mac/iPhone/iPad app that uses 5 advanced AI models to instantly rate, organize, title, describe, tag, and color-code your photos from any folder. 
          Save hours on photo selection and focus on what you do best—creating stunning images.
Start Free Trial Now
Watch Demo
Used by 500+ professional photographers
Powered by 5 Advanced AI Models
badge-announcement
text-hero-headline
text-hero-subheadline
button-start-trial-hero
button-watch-demo
text-user-count

### PricingSection
Choose Your Plan
All plans include a 1-day unlimited free trial
24-Hour Bonus: Get 3 extra months free on annual plans
Professional
Perfect for individual photographers
$99
/month
Billed annually at $1,188/year
Save $396 vs monthly billing
handleSelectPlan(
Start Free Trial
Most Popular
Studio
For studios and high-volume photographers
$499
Billed annually at $5,988/year
Save $2,004 vs monthly billing
All plans can be canceled anytime. No long-term commitments.
section-pricing
text-pricing-headline

### ProblemSection
The Photo Rating Nightmare
Every Photographer Knows
You love capturing moments, not spending hours staring at thumbnails deciding which shots make the cut.
Average photographer spends
200+ hours per year
just rating photos
section-problem
text-problem-headline

### SolutionSection
Introducing
Kull
The first AI-powered photo rating and organization system for Mac, iPhone, and iPad, 
          combining 5 advanced AI models to rate and organize your photos with professional-level accuracy.
Live Demo Coming Soon
See Kull rating photos in real-time
Try It Free for 1 Day
section-solution
text-solution-headline
demo-mockup
button-start-trial-solution

### ValueStack
Everything You Get With Kull
A complete AI-powered photo rating system that saves you hundreds of hours
Core Features
Bonus Features (Included Free)
TOTAL VALUE
$2,400/year
Starting at $99/mo
Save 200+ hours annually
Zero Risk Guarantee
Start with a
1-day unlimited free trial
. 
            Cancel anytime—no questions asked.
section-value-stack
text-value-headline

### FAQSection
Frequently Asked Questions
Everything you need to know about Kull
section-faq
text-faq-headline

### ReferralSection
Refer Photographers,
Unlock Incredible Bonuses
Share Kull with your photography network during signup or checkout and stack rewards as your referrals grow
Referrals
Referral Rewards Timeline
Your referrals get the same amazing free trial, and you get rewarded for spreading the word
section-referral
text-referral-headline

