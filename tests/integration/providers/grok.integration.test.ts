 ...
 *
 * Tests real xAI Grok API (grok-2-vision-1212) with vision support and 1-1000 ratings
 *
 * NOTE: This test requires a valid GROK_API_KEY environment variable
 * Run with: GROK_API_KEY=xai-... npm test
 */
...
describe('Grok Integration Test', () => {
  ...
  it('should process real image with grok-2-vision-1212', async () => {
    if (!hasApiKey) {
      console.warn('Skipping integration test - no GROK_API_KEY found');
      return;
    }
  ...
  it('should calculate cost with correct pricing ($2/$10 per 1M)', async () => {
    if (!hasApiKey) {
      console.warn('Skipping integration test - no GROK_API_KEY found');
      return;
    }
    ...
    // Verify pricing math ($2.00 input, $10.00 output per 1M)
    const expectedInputCost = (result.cost.inputTokens / 1_000_000) * 2.00;
    const expectedOutputCost = (result.cost.outputTokens / 1_000_000) * 10.00;
  ...
  it('should not support batch API', async () => {
    ...
    await expect(adapter.submitBatch({
      ...
      systemPrompt: 'Rate',
      userPrompt: 'Analyze'
    })).rejects.toThrow('Grok does not support batch API');
